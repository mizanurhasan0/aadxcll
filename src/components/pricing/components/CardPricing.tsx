...
const CardPricing: React.FC<CardPricingProps> = ({ data }) => {
    return (
        <div className={`relative p-8 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 ${data.popular ? 'border-2 border-primary' : ''}`}>
            {data.popular && (
